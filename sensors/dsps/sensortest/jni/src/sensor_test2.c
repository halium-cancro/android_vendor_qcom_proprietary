...
{
LOGE( "zhangxiaowei sensorID = %d, dataType = %d, testType = %d,saveToRegistry = %d,applyCalNow = %d", sensorID, dataType, testType ,saveToRegistry,applyCalNow);
  ...
  (JNIEnv *ENV, jclass class, jboolean enabled)
{
   ...
   {
      LOGE( "%s: Error in sensor_reg_close(): %i",
            __FUNCTION__, error );LOGE( "%s: Time-out: %i", __FUNCTION__, test_result );
      return error;
   }
...
registerMethods(JNIEnv* env)
{
   static const char* const kClassName =
      "gn/com/android/mmitest/item/SensorTest";
