  ...
  module_sensor_bundle_info_t * s_bundle, mct_event_control_parm_t *event_parm)
{
  ...
  ret = actuator_module_params->func_tbl.process(
    actuator_module_params->sub_module_private,
    ACTUATOR_GET_AF_TUNE_PTR, &af_tune);
  if ((ret < 0) || (!af_tune)) {
...
//Gionee <zhaocuiqin> <2014-07-12> modify for CR01321247 begin
	   ...
	   if (stream_on_cfg.stream_mask & (1 << CAM_STREAM_TYPE_VIDEO)) {
          ...
          if (ois_module_params != NULL &&
              ois_module_params->func_tbl.process != NULL) {
            rc = ois_module_params->func_tbl.process(
              ois_module_params->sub_module_private,
              ACTUATOR_SET_OIS_MODE, &ois_mode);
          }
#endif
//Gionee <zhaocuiqin> <2014-07-12> modify for CR01321247 endif
...
//Gionee <zhuangxiaojian> <2014-06-26> modify for CR01310542 begin
#ifdef ORIGINAL_VERSION
#else
  case CAM_INTF_PARM_OIS_MODE: {
    int32_t ois_mode = *((int32_t *)event_control->parm_data);
    module_sensor_params_t	*actuator_module_params = NULL;
    actuator_module_params = s_bundle->module_sensor_params[SUB_MODULE_ACTUATOR];
  
    SERR("%s: CAM_INTF_PARM_OIS_MODE is %d.", __func__, ois_mode);
  ...
  case CAM_INTF_PARM_LED_FLASH_BURST_LEVEL: {
    int32_t level = *((int32_t *)event_control->parm_data);
    module_sensor_params_t        *led_module_params = NULL;
    led_module_params = s_bundle->module_sensor_params[SUB_MODULE_LED_FLASH];

    SERR("%s: CAM_INTF_PARM_LED_FLASH_BURST_LEVEL is %d.", __func__, level);

    if (led_module_params->func_tbl.process != NULL) {
        rc = led_module_params->func_tbl.process(
            led_module_params->sub_module_private,
            LED_FLASH_SET_BURST_LEVEL, &level);
    }
    break;
  }
#endif
          ...
          if (stats_get.aec_get.trigger_led)
          {
            ...
            if (led_module_params->func_tbl.process != NULL) {
              ...
              rc = led_module_params->func_tbl.process(
                ...
                LED_FLASH_SET_MAIN_FLASH , NULL);
			  #else
			  int32_t led_burst_level;
			  
			  rc = led_module_params->func_tbl.process(
			    led_module_params->sub_module_private,
			    LED_FLASH_GET_BURST_LEVEL , &led_burst_level);
			  if (rc == SENSOR_SUCCESS) {
				if (led_burst_level == CAM_LED_FLASH_LOW) {
				  SHIGH("%s: set led flash to low for bust shot.", __func__);
				  led_module_params->func_tbl.process(
				  	led_module_params->sub_module_private,
				  	LED_FLASH_SET_PRE_FLASH , NULL);
				} else {
				  SHIGH("%s: set led flash to high for normal shot.", __func__);
				  led_module_params->func_tbl.process(
				  	led_module_params->sub_module_private,
				  	LED_FLASH_SET_MAIN_FLASH , NULL);
				}
