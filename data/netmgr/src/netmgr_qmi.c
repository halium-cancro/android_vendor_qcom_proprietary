...
#define NETMGR_QMI_CLIENT_INVALID            (-1)
  ...
  #define NETMGR_IS_REV_IP_TRANS_CONN_ID(cid)  (0 == strncmp((cid), "rev_rmnet", 9))
...
)
{
  ...
  /* find first link that is active */
  for(i=0; i<NETMGR_MAX_LINK; i++)
  {
  ...
  netmgr_log_high("netmgr_qmi_sys_cb: default link found is %d\n"
                  ,i);
  ...
  {
    netmgr_log_err("netmgr_qmi_sys_cb: no links found active\n");
    netmgr_log_err("netmgr_qmi_sys_cb: sys ind [%d] can not be processed\n",
                   event_id);
...
)
{
  ...
  switch( ind_id ) {
    case  QMI_QOS_SRVC_EVENT_REPORT_IND_MSG:
      /* Set command object parameters */
      cmd->data.type                     = NETMGR_QMI_MSG_CMD;
      cmd->data.link                     = (int)(uint32)user_data;
...
)
{
  ...
  if( QMI_WDS_CURR_CALL_INFO_IPV4_ADDR & call_info.mask ) {
    ...
    {
      addr_info_ptr->if_addr.addr.v4 = htonl( call_info.ipv4_addr );
  ...
  else
  {
    ...
    addr_info_ptr->if_mask = call_info.ipv6_addr_info.ipv6_prefix_len;
  ...
  if( QMI_WDS_CURR_CALL_INFO_SUBNET_MASK & call_info.mask ) {
    addr_info_ptr->if_mask = htonl( call_info.ipv4_subnet_mask );
  ...
  if( QMI_WDS_CURR_CALL_INFO_IPV4_GATEWAY_ADDR & call_info.mask ) {
    addr_info_ptr->gateway.addr.v4 = htonl( call_info.ipv4_gateway_addr );
  ...
  else
  {
    if( QMI_WDS_CURR_CALL_INFO_IPV6_GTWY_ADDR & call_info.mask ) {
      ...
      addr_info_ptr->gw_mask = call_info.ipv6_addr_info.ipv6_prefix_len;
  ...
  if( QMI_WDS_CURR_CALL_INFO_PRIMARY_DNS_IPV4_ADDR & call_info.mask ) {
    ...
    addr_info_ptr->dns_primary.addr.v4 =
      htonl( call_info.primary_dns_ipv4_addr );
  ...
  if( QMI_WDS_CURR_CALL_INFO_SECONDARY_DNS_IPV4_ADDR & call_info.mask ) {
    ...
    addr_info_ptr->dns_secondary.addr.v4 =
      htonl( call_info.secondary_dns_ipv4_addr );
...
)
{
  ...
  for(i=0; i<NETMGR_MAX_LINK; i++)
  {
    ...
    if (netmgr_qmi_cfg.link_array[i].enabled == FALSE ||
        NULL == (cid = netmgr_qmi_get_conn_id_for_link(i)) ||
    ...
    netmgr_log_high("posting sys ind [%d] on link [%d]\n",
                    sm_ev, i);
    ...
    case QMI_WDS_PACKET_DATA_DISCONNECTED:
    {
      ...
      if( QMI_WDS_PKT_SRVC_IND_IP_FAMILY & ind_data->pkt_srvc_status.param_mask )
      {
        netmgr_qmi_cfg.links[ link ].wds_info.addr_info.valid_mask &=
          ...
          ~NETMGR_ADDRSET_MASK_IPV4 : ~NETMGR_ADDRSET_MASK_IPV6;
      }
    }
    break;
     ...
     (QMI_WDS_EVENT_BEARER_TECH_IND & ind_data->event_report.event_mask))
  {
    if ((QMI_WDS_EVENT_BEARER_TECH_EX_IND & ind_data->event_report.event_mask))
    {
      ...
      else
      {
        ...
        {
           if (NETMGR_SUCCESS == netmgr_qmi_query_modem_mtu(link))
           {
...
)
{
  ...
  if (QMI_WDS_REV_IP_TRANS_IND_IPV4_ADDR_PARAM_MASK & rev_ip_ind->param_mask)
  {
    ...
    addr_info_ptr->ipv4.if_addr.addr.v4 = htonl(rev_ip_ind->ipv4_addr);
  ...
  else if (QMI_WDS_REV_IP_TRANS_IND_IPV6_ADDR_PARAM_MASK & rev_ip_ind->param_mask)
  {
    ...
    addr_info_ptr->ipv6.if_mask = rev_ip_ind->ipv6_addr_info.ipv6_prefix_len;
...
)
{
  ...
  netmgr_qmi_cfg.links[ link ].wds_info.addr_info.valid_mask &= (NETMGR_QMI_CLIENT_IPV4 == clnt) ?
                                                                ~NETMGR_ADDRSET_MASK_IPV4 :
                                                                ~NETMGR_ADDRSET_MASK_IPV6;
...
)
{
  ...
  if( cdma_desc->param_mask & QMI_QOS_CDMA_FLOW_PARAM_PROFILE_ID ) {
    if( NETMGR_SUCCESS !=
        netmgr_tc_get_qos_params_by_profile_id( cdma_desc->profile_id,
                                                datarate,
                                                priority ) ) {
...
)
{
  ...
  cmd->data.info.qos_flow.flow_id   = flow_ptr->qos_flow_state.qos_identifier;
  cmd->data.info.qos_flow.is_new    = flow_ptr->qos_flow_state.new_flow;
  cmd->data.info.qos_flow.flow_type = (QMI_QOS_EVENT_REPORT_FLOW_INFO_FLOW_TYPE_PARAM &
                                       flow_ptr->param_mask)?
  ...
  cmd->data.info.qos_flow.num_filter = flow_ptr->tx_filter_count;
  ...
  switch( qmi_cmd->type ) {
    case NETMGR_QMI_SYS_IND_CMD:
      netmgr_sys_ind( qmi_cmd->data.sys_ind.event_id,
                      ...
                      qmi_cmd->data.sys_ind.user_data);
      break;
...
/*=========================================================================*/
LOCAL int
netmgr_qmi_service_init( int link )
{
  char * qmi_cid;
  int qmi_err;
  int wds_clnt_id;
...
{
  if( NETMGR_SUCCESS == netmgr_qmi_verify_link( link ) )
    return netmgr_qmi_cfg.links[ link ].wds_info.mtu;
  else
...
/*=========================================================================*/
void
netmgr_qmi_init (int nlink, netmgr_ctl_port_config_type links[])
{
