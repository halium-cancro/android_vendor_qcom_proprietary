...
{
  qdi_client_handle_t qdi_handle = (qdi_client_handle_t) user_data;
  qdi_call_info_t  *call_info = NULL;
  ds_dll_el_t      *node = NULL;
  ds_dll_el_t      *next = NULL;
  boolean          send_ind = TRUE;
  const void       *dummy = NULL;
      ...
      switch (ind_data->pkt_srvc_status.link_status)
      {
        case QMI_WDS_PACKET_DATA_CONNECTED:
          ...
          else
          {
            ...
            {
              QDI_LOG_MED("ignoring CONNECT pkt_srvc_ind with unexpected ip_family=%d",
                          ind_data->pkt_srvc_status.ip_family);
              send_ind = FALSE;
              continue;
            }
            ...
            if (QDI_MODE_V4V6 == call_info->mode &&
                (call_info->v4_pkt_srvc_conn_ind_cnt + call_info->v6_pkt_srvc_conn_ind_cnt) > 1)
            {
              QDI_LOG_MED("ignoring duplicate connected pkt_srvc_ind for IPV4V6 mode=0x%x call",
                          call_info->mode);
              send_ind = FALSE;
              continue;
            }
...
)
{
  ...
  if (txn->v4_cb_pend == FALSE && txn->v6_cb_pend == FALSE)
  {
    ...
    QDI_LOG_LOW("qdi_cb_hdlr: user_data=0x%x", (unsigned int)txn->user_data);
...
)
{
  ...
  {
    uint32 gw_addr = htonl( call_info.ipv4_gateway_addr );
    sin = (struct sockaddr_in *)&addr->gtwy_addr_s.addr;

    addr->gtwy_addr_s.addr.ss_family = ip_family;
  ...
  {
    sin6 = (struct sockaddr_in6 *)&addr->gtwy_addr_s.addr;
  ...
  {
    uint32 dnsp_addr = htonl( call_info.primary_dns_ipv4_addr );
    sin = (struct sockaddr_in *)&addr->dnsp_addr_s.addr;

    addr->dnsp_addr_s.addr.ss_family = ip_family;
  ...
  {
    sin6 = (struct sockaddr_in6 *)&addr->dnsp_addr_s.addr;
  ...
  {
    uint32 dnss_addr = htonl( call_info.secondary_dns_ipv4_addr );
    sin = (struct sockaddr_in *)&addr->dnss_addr_s.addr;

    addr->dnss_addr_s.addr.ss_family = ip_family;
  ...
  {
    sin6 = (struct sockaddr_in6 *)&addr->dnss_addr_s.addr;
