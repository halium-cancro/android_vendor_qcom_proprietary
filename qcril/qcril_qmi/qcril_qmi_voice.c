        ...
        switch ( iter_call_info->call_state )
        {
          ...
          case CALL_STATE_END_V02:
            ...
            if ( call_info_entry )
            {
              ...
              if ( ( CALL_TYPE_EMERGENCY_V02 == iter_call_info->call_type || CALL_TYPE_EMERGENCY_IP_V02 == iter_call_info->call_type ) &&
                   call_info_entry->elaboration & QCRIL_QMI_VOICE_VOIP_CALLINFO_ELA_CALL_GOT_CONNECTED )
              {
    ...
    */
    ...
    if ( remote_party_name_valid && NULL != remote_party_name )
    {
      ...
      *(entry->voice_svc_remote_party_name.name) = 0;
      ...
      QCRIL_LOG_INFO("remote party name - coding scheme %d, len %d", remote_party_name->coding_scheme, remote_party_name->name_len);
      entry->voice_svc_remote_party_name.name_len = qcril_cm_ss_convert_ussd_string_to_utf8(remote_party_name->coding_scheme,
